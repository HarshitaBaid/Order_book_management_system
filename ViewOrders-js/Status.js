$(document).ready(function () {
    ...
    $(".status svg").on("click", function (event) {
    ...
    $(".status .search-input").on("click", function (event) {
        event.stopPropagation(); // Prevent the click from reaching the document
        $(this).closest(".status").find(".dropdown-list").show();
    });

    // Hide dropdown when clicking outside of it
    $(document).click(function (event) {
        if (!$(event.target).closest('.custom-dropdown, .status .search-input, .status .dropdown-list').length) {
            $(".custom-dropdown").hide();
            $(".status svg").show();
        }
    });
